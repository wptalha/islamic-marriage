...
$output .= '<div class="usof-form-row-control"><div class="usof-font">';
$output .= '<input type="hidden" name="' . $field_id . '" value="' . $value . '" />';
$output .= '<select>';
...
foreach ( $font_weights as $font_weight => $font_title ) {
	...
	$output .= '<input type="checkbox" id="usof_' . $field_id . '_weight_' . $font_weight . '" ';
	$output .= 'value="' . $font_weight . '"';
	$output .= ( in_array( $font_weight, $font_value[1] ) ? ' checked' : '' );
	$output .= '><label for="usof_' . $field_id . '_weight_' . $font_weight . '" ';
