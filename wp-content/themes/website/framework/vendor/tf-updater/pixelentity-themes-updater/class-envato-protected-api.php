...
if ( ! class_exists( 'Envato_Protected_API' ) ) {
	...
	class Envato_Protected_API {
		...
		public function __construct( $user_name = '', $api_key = '' ) {
			...
			if ( $user_name == '' ) {
				$this->set_error( 'user_name', 'Please enter your Envato Marketplace Username.' );
			}

			if ( $api_key == '' ) {
				$this->set_error( 'api_key', 'Please enter your Envato Marketplace API Key.' );
			}
		...
		public function private_user_data( $set = '', $user_name = '', $set_data = '', $allow_cache = false, $timeout = 300 ) {
			...
			if ( $set == '' ) {
				$this->set_error( 'set', 'The API "set" is a required parameter.' );
			}
		...
		public function wp_download( $item_id ) {
			...
			if ( ! isset( $item_id ) ) {
				$this->set_error( 'item_id', 'The Envato Marketplace "item ID" is a required parameter.' );
			}
