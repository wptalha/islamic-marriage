...
//			wp_enqueue_style( 'rwmb-image', RWMB_CSS_URL . 'image.css', array(), RWMB_VER );
		...
		{
			$i18n_title = apply_filters( 'rwmb_image_upload_string', 'Upload Images', $field );
			$i18n_more  = apply_filters( 'rwmb_image_add_string', '+ Add new image', $field );
		...
		{
			$i18n_delete = apply_filters( 'rwmb_image_delete_string', 'Delete' );
			$i18n_edit   = apply_filters( 'rwmb_image_edit_string', 'Edit' );
			$li = '
				<li id="item_%s">
					<img src="%s" />
