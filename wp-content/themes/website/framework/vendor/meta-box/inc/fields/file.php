...
//			wp_enqueue_style( 'rwmb-file', RWMB_CSS_URL . 'file.css', array(), RWMB_VER );
			...
			wp_localize_script( 'rwmb-file', 'rwmbFile', array(
				'maxFileUploadsSingle' => 'You may only upload maximum %d file',
				'maxFileUploadsPlural' => 'You may only upload maximum %d files',
			) );
		}
		...
		static function wp_ajax_delete_file()
		{
			...
			if ( $ok )
				...
				wp_send_json_error( 'Error: Cannot delete file' );
		}
		...
		{
			$i18n_title = apply_filters( 'rwmb_file_upload_string', 'Upload Files', $field );
			$i18n_more  = apply_filters( 'rwmb_file_add_string', '+ Add new file', $field );
		...
		{
			$i18n_delete = apply_filters( 'rwmb_file_delete_string', 'Delete' );
			$i18n_edit   = apply_filters( 'rwmb_file_edit_string', 'Edit' );
			$li = '
			<li>
