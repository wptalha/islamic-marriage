  ...
  <div class="mainin">
    ...
    <div style="width:100%" id="rgt" class="rgt">
      ...
      <div style="float:left; width:100%; padding: 20px 0; font-size:13px;"> <?php $result = mysqli_query($mysqli, "SELECT * FROM login WHERE id=".$_SESSION['id']." ORDER BY id DESC");
        ...
        Opening Balance: <?php $result = mysqli_query($mysqli, "Select cr from transfer WHERE login_id=".$_SESSION['id']." order by id asc Limit 1 ");
...
while($res = mysqli_fetch_array($result)) {		
 ...
 ?>
       ...
       Closing Balance: <?php $result = mysqli_query($mysqli, "Select sum(cr) - sum(dr) as tcr from transfer WHERE login_id=".$_SESSION['id']." ");
...
CROSS JOIN
    transfer AS t
WHERE benid ='0' AND login_id=".$_SESSION['id']."
ORDER BY
    id ASC");
		while($res = mysqli_fetch_array($result)) {		
			echo "<tr>";
